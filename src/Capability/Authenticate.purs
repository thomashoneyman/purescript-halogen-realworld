{- This module describes the capability to read, write, and clear
   credentials stored about a single user in the application. It
   is used for local storage now, but could be switched to another
   source later.
-}
...
{- The ability to read, write, and clear credentials. Includes an
...
instance authenticateHalogenM
  :: Authenticate m
  => Authenticate (HalogenM s f g p o m)
  where
